// A small OData v4 query engine over in-memory rows: just the subset the app sends.
//   resource:  entityset | Function | entityset(<key>)/navigation
//   options:   $select, $filter, $orderby, $top, $expand (names only), $skiptoken (our own offsets)
//   $filter:   eq ne gt ge lt le, and / or / not, parentheses, Microsoft.Dynamics.CRM.In(...)
...
function compareValues(actual: unknown, literal: Literal): number | null {
  ...
  switch (literal.kind) {
    case 'date': {
      const t = typeof actual === 'string' ? Date.parse(actual) : NaN;
      return Number.isNaN(t) ? null : Math.sign(t - literal.value);
    }
    case 'number':
      return typeof actual === 'number' ? Math.sign(actual - literal.value) : null;
    case 'bool':
...
export function evaluate(expr: Expr, row: Row): boolean {
  switch (expr.type) {
    ...
    case 'in':
      return expr.values.some((v) => compareValues(row[expr.field], v) === 0);
    case 'compare': {
      const c = compareValues(row[expr.field], expr.value);
...
export function parseRequest(path: string): ParsedRequest {
  ...
  if (nav) {
    request.key = nav[2]!.replace(/'/g, '').toLowerCase();
...
export function project(row: Row, request: ParsedRequest, annotations: boolean): Row {
  ...
  else for (const key of Object.keys(row)) if (!key.includes(ANNOTATION) && typeof row[key] !== 'object') keep(key);
