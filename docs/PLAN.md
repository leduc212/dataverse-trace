...
> **Status (2026-09-24):** plan agreed. **v0.1, v0.2 and v0.3 are implemented** and waiting to be tested in a real environment (see the milestone table). Decisions are [at the end](#open-decisions).
...
| **M3 → v0.3 "Insights"** ✅ built, to be tested | Beyond debugging | Trends from rollups with coverage bands; platform-stats snapshots (S8); **insight rules**; **cascade graph** with cycle detection; timeline polish (critical path, minimap). **Built:** everything listed. Hourly rollups (400 days) give the dashboard a 90-day range and like-for-like changes against the previous period; retention is applied. 15 insight rules with evidence and per-environment thresholds. Platform statistics keep a snapshot per update, which records S8's refresh cadence as it goes. The cascade graph is custom SVG instead of React Flow and elkjs, and the critical path comes with a time breakdown (engineering §9). | 3–4 weeks |
