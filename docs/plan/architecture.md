...
| R3 | Nesting: a request at depth d+1 is the child of the depth-d plugin span in the same correlation whose `[start, start+duration]` contains the request's first child's start (± a tolerance of 1 s when precision is whole seconds). When there are several candidates, pick the tightest window. **If it's still ambiguous, attach to a synthetic "depth d (ambiguous)" group instead of guessing**, with confidence = 1 / number of candidates. | `childOf` |
| R4 | `asyncoperation.owningextensionid` = `plugintracelog.pluginstepid`, same correlation, trace start within `[startedon, completedon]`. | system job is the parent of the async trace row |
