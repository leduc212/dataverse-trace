...
| S8 | What time window does `plugintypestatistic` cover (rolling? since registration?) and how often is it refreshed? | Decides whether snapshot differences can be read as trends. | Take snapshots every hour for two days. **Since v0.3 the app does this by itself:** it keeps a snapshot whenever a row's `modifiedon` changes, and the platform statistics panel shows the median refresh interval and whether counts ever drop (a rolling window) or only grow (running totals). Leave the app open in a real environment for two days, then read the answer off the panel. |
